#include <cstring> // only for std::memcpy
#include <variant>
#include <vector>
#include <tuple>
#include <limits>
...
varint expand_conn(ReliableConnection conn)
{
  int length = (int)conn.readOne();
...
bool isLittleEndian()
{
    ...
    byte* bytePtr = reinterpret_cast<byte*>(&num);
...
bytes squeeze_floating(std::variant<float, double> value)
{
  if(std::holds_alternative<float>(value))
  {
    ...
    bytes result = bytes();
    result.insert(result.end(), (byte)sizeof(float));
  ...
  else
  {
    ...
    bytes result = bytes();
    result.insert(result.end(), (byte)sizeof(double));
...
{
  int length = (int)value.at(0);

  if(length == 0)
  {
    return maybe<floating>(0.0);
  }
  ...
  switch(length)
  {
    case 4:      
      ...
      return maybe<floating>(f);

    case 8:
      double d;
      std::memcpy(&d, floatBytes.data(), length);
      return maybe<floating>(d);
...
maybe<floating> expand_conn_floating(ReliableConnection conn)
{
  ...
  switch(length)
  {
    case sizeof(float):
    {
      ...
      return maybe<floating>(floating(f));
    }
    ...
    case sizeof(double):
    {
      ...
      return maybe<floating>(floating(d));
    }
...
bytes squeeze_ints(ints values)
{
  // The only case where the length can be zero is for the number 0.
  if(values.size() == 0)
  {
...
std::tuple<ints, bytes> expand_ints(bytes data)
{
  // If input vector is empty, we have nothing to parse
  if(data.size() == 0)
  {
...
bytes squeeze_floats(floats values)
{
  // The only case where the length can be zero is for the number 0.
  if(values.size() == 0)
  {
